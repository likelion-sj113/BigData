...
public class CustomerManager3 {
	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			case 'i':
				...
				custList = insertCustomerData(scan, custList);
			...
			case 'p' :
				...
				if(index <= 0) {
					...
					index--;
					printCustomerData(index,custList);
				}
				break;
			case 'n' :
			...
			case 'u' :
				...
				if( (index >= 0) && (index < count)) {
					...
					updateCustomerData(index,custList, scan);
				}else {
			...
			case 'd' :
				...
				if( (index >= 0) && (index < count)) {
					...
					deleteCustomerData(index, custList);
				}else {
	...
	public static ArrayList<Customer> insertCustomerData(Scanner scan, ArrayList<Customer> custList) {
		System.out.print("이름 : ");	
...
//		//고객 객체를 ArrayList에 저장
		//custList.add(cust);
		CustomerInsertData custinser = new CustomerInsertData();
		custList = custinser.insertCustomerData(cust, custList);
		return custList;
	...
	public static void printCustomerData(int index, ArrayList<Customer> custList) {
		CustomerPrint.printCustomerData(index, custList);
	}

	//index 위치의 고객정보를 삭제합니다.
	public static void deleteCustomerData(int index, ArrayList<Customer> custList) {
		custList.remove(index);
	}

	//index 위치의 고객 정보를 수정합니다.
	public static void updateCustomerData(int index, ArrayList<Customer> custList, Scanner scan) {
